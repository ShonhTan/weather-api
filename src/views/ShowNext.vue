<template>
  <div v-if="show" class="showNext p-3 bg-light text-dark border rounded">
    ...
    <div v-if="info!==[]" class="showNext-info">
      <show-next-row v-for="(dayinfo, index) in info"
                     :key="index"
                     :day-info="dayinfo"
...
export default {
  ...
  methods:{
    ...
    APICallNext () {
      this.$http.get(`https://api.openweathermap.org/data/2.5/forecast?q=${store.city}&APPID=${store.APIKey}&units=metric`)
      .then( (response) => {
  ...
  mounted(){
    ...
    if( (!store.next && store.city !== "") || (store.cod == 200 && store.next.city.toLowerCase() !== store.city.toLowerCase()) ) {
      this.APICallNext()
